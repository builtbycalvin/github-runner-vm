...
| Shared profile reserved, guest setup interrupted | Preserve the exact profile and setup gate. Complete the named member preparation, then run the selected profile's authenticated `register` transaction. | Delete the reservation, overwrite another registration, request a copied token, or resume around the gate. |
