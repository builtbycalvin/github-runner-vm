...
- `ci-vm --repo other/second register --all-repos` after approval covering every repository sharing that VM.
...
- A shared profile is a reservation, not a registered runner. Successful `register --all-repos` performs exact GitHub readback, persists the runner ID, enables the inactive unit, and completes the matching setup gate. A stale or missing member in the guest inventory must block maintenance.
